...
With the attached package, you can simply start the application by running `GeDi()`. This will launch the application and land you on the Welcome page. Via the Data Input panel you can simply provide your data directly to the application. This will also ensure that your data is in the right input format. Another way to start the applicationis to run:
...
This panel will handle the input of the data if not provided in the function call. 

The panel is divided into boxes which will correspond to the individual steps of the data input. The individual boxes will appear consecutively once the previous step has been completed successfully. 

In the first step, you can provide you data via the Browse button. This will open a modal window to your computer storage where you can select the respective file with your input data. Once the data has successfully been read in, you can have a look at you data via the Data Preview box on the right. 
During this step, the application will check if your input contains the columns Genesets and Genes. If these columns are not found, the application will show a small error message in the lower right corner and show two drop down menus in which you can select the correct columns of your data. 
